...
def main():
    ...
    # fig4: PCA spectrum only (intrinsic-dimension panel dropped — it merely recovers
    # the known 3-D dimensionality of the stimulus; the estimate is reported in text).
    fig, a1 = plt.subplots(figsize=(7.4, 5.2))
    for name in layers:
        r = pca_res[name]
        a1.plot(np.arange(1, len(r["cum"]) + 1), r["cum"], color=colors[name],
                lw=2.2, label=f"{name}: 95% при {r['n95']} комп., PR={r['pr']:.0f}")
    ...
    a1.legend(frameon=False, fontsize=13, loc="lower right")
    a1.grid(color="#EEE")
    fig.tight_layout()
    for ext in ("png", "pdf"):
    ...
    fig, (b1, b2, b3) = plt.subplots(1, 3, figsize=(14.4, 4.8))
    for name in layers:
        S = spec[name]
        b1.plot(np.arange(1, len(S) + 1), S / S[0], color=colors[name], lw=2.2, label=name)
    b1.set_xscale("log"); b1.set_yscale("log")
    b1.set_xlabel("Номер сингулярного числа (log)")
    b1.set_ylabel("σᵢ / σ₁ (log)")
    b1.legend(frameon=False, fontsize=13); b1.grid(color="#EEE")

    for name in layers:
        b2.plot(ranks, curves[name], "o-", color=colors[name], lw=2.2, label=name)
    b2.axhline(0.288, color=GRAY_D, ls="--", lw=1.2)
    b2.text(ranks[-1], 0.298, "CAM16-LCD (0.288)", ha="right", color=GRAY_D, fontsize=12)
    b2.set_xscale("log")
    b2.set_xlabel("Ранг усечённого отображения r (log)")
    b2.set_ylabel("STRESS на тесте (group-k)")
    b2.legend(frameon=False, fontsize=13); b2.grid(color="#EEE")

    for name in layers:
        A, _ = A_store[name]
        cn = np.sort(np.linalg.norm(A, axis=0))[::-1]
        b3.plot(np.arange(1, len(cn) + 1), cn / cn[0], color=colors[name], lw=1.8, label=name)
    b3.set_xscale("log")
    b3.set_xlabel("Входные измерения эмбеддинга (log)")
    b3.set_ylabel("Норма столбца A (отн.)")
    b3.legend(frameon=False, loc="lower left", fontsize=13); b3.grid(color="#EEE")
    axins = b3.inset_axes([0.45, 0.45, 0.5, 0.45])
    axins.hist(A_store["VL"][0].ravel(), bins=120, color=ACCENT_D, alpha=0.85)
    axins.text(0.5, 1.02, "веса A (VL)", transform=axins.transAxes,
